  ...
  describe('when methods dealing with connection to the handler are stubbed', () => {
    ...
    it('should include the status in the error if spawned process ends with non-zero exit status', (done) => {
      ...
      loadWorkerClient((workerError) => {
        ...
        // The spawned handler exits with a non-zero status on its own. Wait
        // until that exit has been observed (and 'hookHandlerError' set)
        // before stopping the client. Stopping first would mark the
        // termination as intentional, which suppresses crash detection and
        // makes this assertion race the process exit.
        const deadline = Date.now() + 5000;
        const waitForCrash = () => {
          if (runner.hookHandlerError) {
            return hooksWorkerClient.stop((stopError) => {
              if (stopError) {
                return done(stopError);
              }
          ...
          if (Date.now() > deadline) {
            return done(
              new Error('Timed out waiting for the hooks handler to crash'),
            );
          }
          return setTimeout(waitForCrash, 10);
        };
        waitForCrash();
      });
    });
